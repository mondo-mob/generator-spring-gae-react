...
export const inviteUser = (userInvitation) =>
  asyncAction('INVITE_USER', usersApi.invite(userInvitation), {
    responseTransformer: (user) => normalize(user, schemas.user),
  });

export const fetchUsers = () =>
  asyncAction('FETCH_USERS', usersApi.list(), {
    responseTransformer: (users) => normalize(users, schemas.arrayOfUsers),
  });

export const fetchUser = (id) =>
  asyncAction('FETCH_USER', usersApi.get(id), {
    responseTransformer: (users) => normalize(users, schemas.user),
  });
